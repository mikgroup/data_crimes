...
for j in range(sampling_type_vec.shape[0]):
    ...
    title_str = 'FatSatPD knee N_images={}'.format(num_slices) + ' ' + 'R={} '.format(R) + samp_label + ' - CS recon error vs lamda'
    plt.title(title_str)
    ax = plt.gca()
    #ax.set_xscale('log')
...
#         samp_label = 'VD strong'
    ...
    plt.plot(pad_ratio_vec,CS_NRMSE_optimal_lamda[:,j],label='lamda calibrated for each ImSize',linestyle='solid',color='k',marker='o')
    plt.plot(pad_ratio_vec, CS_NRMSE_fixed_lamda[:, j],label=f'fixed lamda {lamda_fixed} (paper fig)',linestyle='solid',color='r', marker='s')
