'''
This scripts performs the experiments presented in Fig 8c. It takes networks that were trained (for Fig7) on processed
data, and tests them using non-processed data.

Before running this script, you should update the following:
  trained_models_path - it should be the path of your repository + crime_2_jpeg/Fig7_jpeg_statistics/DL/
...
sys.path.append("../Fig7_jpeg_statistics/DL/")  # add folder for MoDL_single - the trained networks should be there

from subtle_data_crimes.crime_2_jpeg.Fig7.DL.utils.datasets import create_data_loaders
from subtle_data_crimes.crime_2_jpeg.Fig7.DL.utils import complex_utils as cplx
from subtle_data_crimes.crime_2_jpeg.Fig7.DL.MoDL_single import UnrolledModel
from subtle_data_crimes.functions.error_funcs import error_metrics

# add path for the networks that were trained on JPEG compressed data
trained_models_path = '/mikQNAP/efrat/1_inverse_crimes/2_public_repo_mirror_PyCharm/subtle_data_crimes/crime_2_jpeg/Fig7/DL/'
...
# To estimate the crime impact, all the networks will be tested on data that was NOT jpeg-compressed.
# The index for this dataset is 999.
q_test = 999
...
for r in range(R_vec.shape[0]):
    ...
    for qi in range(q_vec.shape[0]):
        ...
        params.data_path = basic_data_folder + data_type + "/q" + str(q_test) + "/" + im_type_str + "/"
        ...
        checkpoint_file = trained_models_path + 'R{}_q{}/checkpoints/model_{}.pt'.format(R, q, checkpoint_num)
...
results_filename = 'Res_MODL_JPEG_exp_crime_impact.npz'
