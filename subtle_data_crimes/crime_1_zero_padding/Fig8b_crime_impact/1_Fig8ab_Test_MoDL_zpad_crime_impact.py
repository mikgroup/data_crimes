            ...
            for i_batch, data in enumerate(test_loader):
                ...
                for i in range(params.batch_size):
                    ...
                    if (i_batch >= 5) & (i_batch <= 15) & (i <= 3):
                        ...
                        plt.title('target')
                        plt.suptitle(
                            'Results R{} pad_x{} MoDL{} - example {}'.format(R, pad_ratio_str, checkpoint_num, i))
                        plt.show()
                        fig.savefig('test_figs_R{}/test_images_pad_x{}_MoDL{}_example{}'.format(R, pad_ratio_str,
                                                                                                checkpoint_num, i))
                        ...
                        plt.suptitle(
                            'Results R{} pad_x{} MoDL{} - example {}'.format(R, pad_ratio_str, checkpoint_num, i))
                        plt.show()
                        fig.savefig('test_figs_R{}/test_images_pad_x{}_MoDL{}_example{}_zoom'.format(R, pad_ratio_str,
                                                                                                     checkpoint_num, i))
...
for v_i in range(var_dens_type_vec.shape[0]):
    ...
    plt.errorbar(pad_ratio_vec, NRMSE_test_set_av[:, v_i].squeeze(), yerr=NRMSE_test_set_std[:, v_i].squeeze(),
                 linestyle='solid', label=label_str,
                 marker=markers[v_i])
...
for v_i in range(var_dens_type_vec.shape[0]):
    ...
    plt.errorbar(pad_ratio_vec, SSIM_test_set_av[:, v_i].squeeze(), yerr=SSIM_test_set_std[:, v_i].squeeze(),
                 linestyle='solid', label=label_str,
                 marker=markers[v_i])
