...
def get_args():
    ...
    parser.add_option('--data_type', '--data_type', type='str', default='val', help='train/val/test/pathology_1/pathology_2')
...
if __name__ == '__main__':
    ...
    while ns<num_slices:
        ...
        for s_i in range(n_slices_in_scan):
            ...
            plt.suptitle(f'R={int(R)} {samp_type} VD samp; pad {pad_ratio}; subject {t} ; slice {s_i}; {data_type} data; \n lamda={lamda}, sparsity_level={nnz_lamda}, block size {block_shape}, num_atoms {num_filters}')
            plt.colorbar(shrink=0.5)
            plt.clim(0, cmax)
            plt.show()
