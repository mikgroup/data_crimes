...
from subtle_data_crimes.functions.utils import zpad_merge_scale,extract_block
                ...
                for im_type_i in range(im_type_vec.shape[0]):
                    ...
                    for i, pad_ratio in enumerate(pad_ratio_vec):
                        ...
                        for s_i in range(n_slices_to_store):
                            ...
                            if (pad_ratio==1) & ((data_type=='pathology_1') | (data_type=='pathology_2')):

                                fig = plt.figure()
                                plt.imshow(np.rot90(np.abs(im_mag_scaled),2),cmap="gray")
                                plt.axis('off')
                                plt.title(f'{subject_str} slice #{s}')
                                plt.show()

                                out_folder = f'figs_{data_type}'
                                if not os.path.exists(out_folder):
                                    os.makedirs(out_folder)

                                figname = out_folder + f'/slice{s}'
                                plt.savefig(figname, dpi=1000)

                                # save eps
                                figname_eps = out_folder + f'/slice{s}.eps'
                                fig.savefig(fname=figname_eps,
                                            format='eps', dpi=1000)
                            ...
                            if create_blocks_flag == 0:  # process full-fove images (merge multi-coil, zero-padd, scale, and store the data)
                                ...
                                fig = plt.figure()
                                plt.imshow(np.rot90(np.abs(im_block),2),cmap="gray")
                                plt.title('im_block')
                                plt.show()
                                print('')
                            ...
                            coil_i = 5
                            print(f'file {filename_h5} - slice {s} coil {coil_i}')

                            ksp_coil = kspace_orig[s,coil_i, :,:].squeeze()
                            fig = plt.figure()
                            plt.imshow(np.log(np.abs(ksp_coil)),cmap="gray")
                            plt.suptitle(f'FastMRI {data_type} data - file {t}, slice {s}, coil {coil_i}')
                            ...
                            if (s_i == 0) & (n_PD_scans == 1):
                                ...
                                plt.subplot(1, 3, 2)
                                plt.imshow(np.rot90(np.abs(img),2), cmap="gray")
                                plt.colorbar(shrink=0.4)
                                plt.title('img / block')
