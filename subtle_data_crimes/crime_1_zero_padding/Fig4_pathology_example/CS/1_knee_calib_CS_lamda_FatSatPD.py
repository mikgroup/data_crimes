...
data_filename = 'knee_lamda_calib_R{}_num_slices{}_Nsamp{}_FatSat'.format(R_vec[0], num_slices,
                                                                          sampling_type_vec.shape[0])
...
NRMSE_arr = np.empty(
    [num_slices, pad_ratio_vec.shape[0], R_vec.shape[0], sampling_type_vec.shape[0], lamda_vec.shape[0]])
...
for pad_i, pad_ratio in enumerate(pad_ratio_vec):
    ...
    while ns < num_slices:
        ...
        for s_i in range(n_slices_in_scan):
            ...
            gold_dict[
                pad_i, ns - 1] = rec_gold  # store the results in a dictionary (note: we use a dictionary instead of a numpy array beause
            ...
            for r in range(R_vec.shape[0]):
                ...
                for j in range(sampling_type_vec.shape[0]):
                    ...
                    for lam_i in range(lamda_vec.shape[0]):
                        ...
                        # print('CS rec...')
                        rec = mr.app.L1WaveletRecon(kspace_sampled, virtual_sens_maps, lamda=lamda,
                                                    show_pbar=False).run()
                        ...
                        plt.colorbar(shrink=0.25)
                        plt.suptitle(
                            f'{data_type} data; R={R}; pad_ratio={pad_ratio}; {samp_type} VD samp; scan {t}; slice {ns}')
                        plt.show()
