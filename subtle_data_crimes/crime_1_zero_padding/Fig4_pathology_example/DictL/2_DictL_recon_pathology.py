...
def get_args():
    ...
    # new in version 5:
    parser.add_option('--pad_ratio', '--pad_ratio', type='float', default=1, help='zero-padding ratio (preprocessed data will be chosen accordingly)')
    parser.add_option('--lamda', '--lamda', type='float', default=0.01, help='lamda (tradeoff between data consistency and regularization terms')
    ...
    parser.add_option('--data_type', '--data_type', type='str', default='pathology_1', help='train/val/test/pathology_1/pathology_2')

    parser.add_option('--num_cpus', '--num_cpus', type='int', default='1',help='number of CPUs that will be employed')

    parser.add_option('--logdir', default="./", type=str, help='log dir') # this is useful for sending many runs in parallel (e.g. for parameter calibration)
...
if __name__ == '__main__':
    ...
    while ns<num_slices:
        ...
        for s_i in range(pathology_slice.shape[0]):
            ...
            gold_dict[ns-1] = rec_gold  # store the results in a dictionary (note: we use a dictionary instead of a numpy array beause
            ...
            plt.suptitle(f'R={int(R)} {samp_type} VD samp; pad {pad_ratio}; subject {t} ; slice {s}; {data_type} data; \n lamda={lamda}, sparsity_level={nnz_lamda}, block size {block_shape}, num_atoms {num_filters}')
            plt.colorbar(shrink=0.5)
            plt.clim(0, cmax)
            plt.show()
